...
class CaseProcessor:
	def __init__(self, dataset: str='dataset_b', graph_builder: str='structural', metric_filters: List[str]=[], graph_filters: List[str]=[], ranker: str='latentregressor'):
		# Initialize
...
def main(dataset: str, graph_builder: str, metric_filter: List[str], graph_filter: List[str], ranker: str, cpus: int):
	...
	processes = []
	for i in range(cpus):
		p = mp.Process(target=worker, args=(i, task_queue, result_queue, dataset, graph_builder, metric_filter, graph_filter, ranker))
		processes.append(p)
		p.start()
